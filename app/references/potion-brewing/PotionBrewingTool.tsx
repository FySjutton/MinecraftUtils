...
function RowView({ icon, label, size, right }: { icon: string; label: string; size: "small" | "large"; right?: React.ReactNode }) {
    return (
        <div className="flex items-center rounded-md border py-2 pl-2">
            <Image src={icon} alt={label} width={size === "large" ? 36 : 24} height={size === "large" ? 36 : 24} className={`${size === "small" ? "mr-[14]" : "mr-[2]"}`} />
            <div className="flex-1 flex items-center justify-between">
                <span className={`text-sm ${size === "large" ? "font-semibold" : "text-xs"}`}>{label}</span>
                {right}
            </div>
        </div>
    )
...
export default function PotionBrewingTool() {
    ...
    const rows = React.useMemo(() => {
        ...
        return out.filter((r, i) => i === 0 || r.type !== out[i - 1].type || r.name !== out[i - 1].name)
    ...
    const steps = React.useMemo(() => {
        const result: { start: PotionName; ingredient: IngredientName; product: PotionName; type: "normal" | "splash" | "lingering" }[] = []
        let i = 0
        while (i < rows.length - 1) {
            const startPotion = rows[i].type === "potion" ? rows[i].name : ""
            const ingredient = rows[i + 1].type === "ingredient" ? rows[i + 1].name : ""
            const product = rows[i + 2]?.type === "potion" ? rows[i + 2].name : ""
        ...
        if (potionType === "splash" && result.length) {
            ...
            result.push({ start: lastProduct, ingredient: "gunpowder", product: `Splash ${lastProduct}`, type: "splash" })
        } else if (potionType === "lingering" && result.length) {
            const lastProduct = result[result.length - 1].product
            const splashName = `Splash ${lastProduct}`
            result.push({ start: lastProduct, ingredient: "gunpowder", product: splashName, type: "splash" })
            result.push({ start: splashName, ingredient: "dragon_breath", product: `Lingering ${lastProduct}`, type: "lingering" })
        }
        ...
        <div className="w-full mx-auto space-y-6">
            <Card>
                ...
                <CardContent>
                    ...
                    <div className="mt-4">
                        <Tabs value={potionType} onValueChange={(v) => setPotionType(v as SetStateAction<"normal" | "splash" | "lingering">)}>
                            <TabsList>
                    ...
                    {steps.map((s, idx) => {
                        ...
                        return (
                            <div key={idx} className="rounded-lg border bg-muted/30 p-4 space-y-2">
                                <div className="text-sm font-medium text-muted-foreground">Step {idx + 1}</div>

                                <RowView
                                    icon={potionIcon(s.start.replace(/^(Splash |Lingering )/, ""), startIconType)}
                                    label={`Start with: ${s.start}`}
                                    size="small"
                                />
                                <RowView
                                    icon={ingredientIcon(s.ingredient)}
                                    label={`Add ${toTitleCase(s.ingredient.replaceAll("_", " "))}`}
                                    size="large"
                                />
                                <RowView
                                    ...
                                    right={
                                        showTabs ? (
                                            <Tabs
                                                ...
                                                onValueChange={(v) => setOptions((o) => ({ ...o, [s.product]: Number(v) }))}
                                            >
                                                <TabsList>
