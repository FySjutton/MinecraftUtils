...
import {TRANSPARENT_GROUP_ID} from '../utils/constants';
...
// Bellman-Ford to find the lowest possible Y values satisfying all height constraints.
// This is what makes valley staircasing stay close to the ground.
...
export function computeColumnY(
    ...
    for (let i = 0; i < n; i++) {
        if (groupIds[i] !== 11) {
            if (brightnesses[i] === Brightness.HIGH) {
                y++;
            } else if (brightnesses[i] === Brightness.LOW) {
                y--;
            }
        }
        rawY.push(y);
...
// write the final brightness/groupId/Y values for a column into the output maps
// Valley mode recomputes Y here while other modes can pass pre-computed Y directly
export function finalizeColumn(
    ...
    if (staircasingMode === StaircasingMode.SOUTHLINE) {
        ...
        for (let z = height - 1; z >= 0; z--) {
            if (colGroupId[z] !== TRANSPARENT_GROUP_ID) {
                southY = colY[z];
                break;
            }
        }
